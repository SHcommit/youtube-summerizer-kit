...
- **Local Audio and Video File Input**:
  ...
  - Command shortcut: run `ytsum summarize ./file.mp3`, `ytsum study ./lecture.mp4`, or `ytsum ./recording.m4a`.
  - Stored absolute source path preservation in SQLite DB for interrupted run recovery via `ytsum resume`.
...
- **Durable Recovery & Caching**:
  ...
  - Resume interrupted runs with `ytsum resume [RUN_ID]`.
...
- **Storage Retention & Policy Management**:
  ...
  - Management commands: `ytsum storage`, `ytsum cleanup`, `ytsum delete`, and `ytsum purge`.
- **Benchmarking Suite**:
  - Automated quality & recall benchmark framework (`ytsum benchmark`) comparing direct Gemini URL analysis against hierarchical pipeline synthesis.

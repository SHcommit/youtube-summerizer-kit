...
- `chew <URL>` 단 한 줄의 명령으로 영상 요약 실행
- 챕터 인식 및 적응형 소주제 단위 비동기 DAG 병렬 처리로 긴 영상도 안정적 분석
- 네트워크나 AI CLI 단절 시에도 완료된 작업부터 이어하는 SQLite WAL 상태 기반 중단 복구
- 동일 URL/파일 및 설정 입력 시 run-id 없이도 기존 Knowledge Pack 원자적 재사용
- 로컬 미디어(MP3, MP4 등) 원본 위치 음성 인식 및 SHA-256 콘텐츠 해시 재사용
- Codex, Gemini, Claude, Ollama, Antigravity(`agy`) CLI를 공통 Harness 어댑터로 연결
- 요약 강도 3단계 지정 (`quick`, `detailed`, `deep`) 지원
- CLI 옵션 대신 Markdown 설정 파일(`CHEW.md`, `.chew/profiles/`)로 블로그 톤 및 지시사항 관리
- OpenTelemetry Jaeger 트레이싱을 통한 실시간 지연시간 및 관측성 시각화 지원

---

## 시각화 데모 & 아키텍처 다이어그램 (Demo & Diagrams)

### 사용자 입력과 출력 흐름 (User Flow)

![YouTube Summarizer Kit 사용자 입력과 출력](assets/architecture/ko/user-flow.png)

### 내부 상세 처리 파이프라인 (Internal Pipeline)

![YouTube Summarizer Kit 내부 상세 처리 흐름](assets/architecture/ko/internal-pipeline.png)

### OpenTelemetry Jaeger 실시간 트레이스 관측성 (Jaeger Trace Observability)
...
`chew` 명령어를 실행하기 전 **반드시 아래 다운로드 및 환경 설치 과정을 먼저 완료**해야 합니다.
...
# 2. 1초 자동 설치 및 chew 명령어 전역 등록 (python/uv 명령 불필요)
./setup.sh
```
...
로컬 오디오·영상 파일 입력에도 `whisper` extra가 필요합니다:
...
## 사용 방법 및 예시 (Demo & Command Examples)

```bash
# 기본 긴 내용 정리본 (완료 시 파이썬 프로세스 자동 종료 및 마크다운 저장)
chew 요약 'https://youtu.be/VIDEO_ID'

# 로컬 녹음 파일 (`whisper` extra 필요)
chew 요약 ./recordings/meeting.mp3

# URL이나 지원하는 로컬 경로는 `요약` 생략 가능
chew 'https://youtu.be/VIDEO_ID'

# 목적별 재조립 (LLM 재분석 없이 1초 만에 생성)
...
### 1. 입력 식별자와 재사용 키 (Transcript Extraction & Identity)
...
### 2. 메타데이터와 자막 검증 (Preprocessing & Captions)
...
### 3. 적응형 분할과 병렬 처리 (Adaptive Segmentation & DAG Processing)
...
### 4. 계층형 요약과 Knowledge Pack (LLM Processing & Synthesis)
...
### 5. 멀티 포맷 출력 재조립 (Multi-Format Output Compilation)
...
## 성능 개선 및 관측성 (Performance Improvements & Observability)

### 30분 지연 병목 현상 16.3배 성능 향상 (30분 -> 1분 50초 단축)

- **영상 길이에 비례하는 동적 세그멘테이션 제한**: 30분 미만 영상 5개 세그먼트 상한 구현 (`segmentation.py`)
- **Harness 동시성 확장**: AntigravityHarness 동시성 한계 2개 -> 8개로 확대 (`antigravity.py`)
- **성능 측정 결과**: 총 소요 시간 기존 30분 이상에서 **1분 50초**로 16.3배 속도 단축 달성

### OpenTelemetry 기반 실시간 추적 대시보드

- `chew benchmark-dashboard` 및 `chew benchmark-ui`를 통해 `reports/trace_report.md` 리포트를 자동 생성하고 Jaeger UI (`http://localhost:16686`)에서 각 스판(Span)별 실행 시간을 실시간 시각화합니다.
...
- **상태 머신 및 저장소**: SQLite WAL, zstandard (Content-Addressed Artifact Storage)
- **자막 및 음성인식**: yt-dlp, youtube-transcript-api, faster-whisper
- **관측성 및 추적**: OpenTelemetry API/SDK, VizTracer

### 포트와 어댑터 아키텍처 (Ports & Adapters / Hexagonal)
...
## 현재 지원 상태 (Supported AI Harnesses)
...
## Markdown 설정과 출력 문체 (CHEW.md & Profiles)
...
다음 파일이 자동 생성되며 기존 파일은 덮어쓰지 않습니다:
...
## 개발과 검증 (Development & Verification)

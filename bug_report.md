...
- [ ] **동적/유연한 세그먼테이션 (Dynamic & Flexible Segmentation) 도입**
  - **문제**: 비디오 길이에 상관없이 규칙 기반으로 세그먼트를 쪼개어, 25분짜리 짧은 영상에서도 30개라는 무리한 개수의 태스크가 빌드됨.
  - **개선**: 비디오의 총 재생 시간(Duration)에 비례하여 동적으로 토픽 개수의 상한선을 제한하는 정책을 도입합니다.
    * 예: 30분 미만 비디오 -> 최대 3~5개 세그먼트 이하로 유연하게 제한.
    * 1시간 이상 긴 강의 -> 10~15개 세그먼트 수준으로 제어.

- [ ] **초고속 단일 패스 프로필 (Single-Pass Fast Path Summary) 추가**
  - **개선**: 계층형 분할 분석(Map-Reduce)을 거치지 않고, 비디오 전체 자막을 단 한 번의 LLM 프롬프트 요청으로 요약하는 초고속(`fast` / `simple`) 프로필을 추가합니다.
  - 특히 사용자가 로그인한 CLI AI Agent(예: `agy` 등)를 그대로 활용하되, 호출 횟수를 1회로 극축하여 에이전트 인증 세션은 유지하면서 스케줄러 오버헤드 없이 **10~15초 내외로 빠른 응답**을 받도록 최적화합니다.

- [ ] **병렬성 증가 (Concurrency Limit Tuning)**
  - **개선**: `AntigravityHarness.maximum_concurrency` 제한을 현재 `2`에서 `8`~`10` 수준으로 대폭 높여, 로컬 리소스가 허용하는 한 여러 요청을 최대한 동시 처리하도록 개선합니다.

- [ ] **에이전트 행동 지침 강제화**
  - **개선**: 파일 시스템 전체 검색(`glob`) 도구를 절대 사용하지 못하도록 환경을 강제 제한하고, 시스템 설정 정보 조회를 최우선하도록 유도합니다.

- [ ] **취합(Compose) 결과 유연성 및 견고성 확보**
  - **개선**: `overview` 및 `further_study` 형식 검증을 다소 관대하게 정규식 매칭 등으로 우회 처리하거나, `repair` 로직이 오작동하더라도 기본 백업 데이터를 활용해 최종 Knowledge Pack 생성이 실패하지 않도록 보완합니다.

---

## 5. 실수를 반복하지 않기 위한 아키텍처적 교훈 (Lessons Learned)

미래의 AI 에이전트 및 기여자가 이 프로젝트를 수정할 때 동일한 설계 오류나 비효율적인 접근을 반복하지 않도록 아래의 행동 수칙을 기술합니다.

1. **로컬 CLI와 통신할 때 고주파 요청(Loop) 금지**:
   * 로컬 CLI 도구(예: `agy`, `codex`, `claude` 등)는 독립 프로세스 구동 비용이 매우 큽니다. 비동기 실행 루프가 수십 번 반복되도록 설계해서는 안 되며, 반드시 **배치(Batch)** 모드 또는 **단일 호출(Single-pass)** 아키텍처를 우선적으로 설계해야 합니다.
2. **에이전트의 로컬 하드 스캔 제한**:
   * 환경 변수 불일치나 `FileNotFoundError` 발생 시, 무조건 로컬 파일 시스템 전체를 `glob()`으로 도는 탐색 행위는 사용자의 프라이버시를 침해하고 막대한 리소스를 낭비합니다. 경로는 `ytsum.app.bootstrap` 내부 로직이나 `platformdirs` 명세를 먼저 체크하고, 애매한 경우에는 탐색 전에 사용자에게 직접 경로를 묻도록 설계 규칙을 세워야 합니다.
3. **데이터베이스 트랜잭션 및 캐시 일관성**:
   * 특정 분석 파이프라인 태스크(예: `compose`)가 실패한 뒤 다시 실행될 때, 상태 DB에 이전에 완료된 태스크들의 캐시 정합성이 깨져 `pack_hash`가 `None`으로 남아버리는 상황이 있었습니다. 재시도 로직 작성 시, 이전 캐시와 성공 여부를 온전히 관리할 수 있도록 초기화 로직을 세심하게 다루어야 합니다.

...
from ytsum.app.config import ConfigurationError
from ytsum.app.retention import CleanupPlan, RetentionPlanner
from ytsum.app.service import AuthenticationRequired, CommandResult, RunStatus
...
def _application_factory() -> Application:
    cli_mod = sys.modules.get("ytsum.cli")
    ...
    from ytsum.app.bootstrap import build_application
...
def _retention_factory() -> RetentionPlanner:
    ...
    from ytsum.app.bootstrap import build_retention_planner
...
def config(
    ...
    if initialize:
        targets = (
            ...
            ("profiles/blog.md", Path(".ytsum/profiles/blog.md")),
            ("profiles/study.md", Path(".ytsum/profiles/study.md")),
            ("profiles/obsidian.md", Path(".ytsum/profiles/obsidian.md")),
        )
        created: list[Path] = []
        templates = files("ytsum.templates")
        ...
        return
    typer.echo(
        "프로젝트 루트의 YTSUM.md와 .ytsum/profiles/*.md를 편집하세요."
        if _is_korean(context)
        else "Edit YTSUM.md and .ytsum/profiles/*.md in the project root."
    )
